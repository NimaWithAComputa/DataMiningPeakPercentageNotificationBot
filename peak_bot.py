...
- Monitors only during active windows (CST):
...
ALERT_THRESHOLD = 95.0   # Send alert when percentage exceeds this
...
def is_active_window() -> bool:
    """
    Returns True if the current CST time falls within a monitoring window.
    ...
    month = now.month
    hour  = now.hour + now.minute / 60   # fractional hour, e.g. 14.5 = 2:30pm
 
    if 6 <= month <= 9:
        # June – September: 11am to 9pm
        active = 11.0 <= hour < 21.0
    else:
        # October – May: 6am–11am OR 5pm–9pm
        active = (6.0 <= hour < 11.0) or (17.0 <= hour < 21.0)
 
    log.info(
        f"Time check: {now.strftime('%b %d %I:%M %p CST')} — "
...
def build_alert(pct: float, now: datetime) -> str:
    bar    = build_bar(pct)
    ts     = now.strftime("%b %d, %Y  %I:%M %p CST")
    return (
...
def main():
    ...
    if not is_active_window():
        # If we were previously alerted, reset so we can alert fresh next window
    ...
    if pct > ALERT_THRESHOLD:
        if not previously_alerted:
            ...
            send_telegram(build_alert(pct, now))
            state["alerted"] = True
        else:
            # Still above threshold — send an update every run so you stay informed
            send_telegram(build_alert(pct, now))
    else:
        if previously_alerted:
            # Just dropped back below — send all clear
            send_telegram(build_all_clear(pct, now))
        state["alerted"] = False
